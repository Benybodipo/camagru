	...
	if ($validate::requestExists() && $_SERVER["REQUEST_METHOD"] == "POST")
	{
		...
		if ($action == "like")
		{
			...
			if ($liked == 1)
			{
				...
				{
					$message = $session->get("loggedin") . " has liked your post";
					$email = $auth["email"];
					sendMail($email, "NEW LIKE", $message);
				}
			}
			else {
				$crud->delete([
		...
		elseif ($action == "comment")
		{
			...
			if ($auth["notification"])
			{
				...
				// sendMail("benybodipo@gmail.com", "NEW COMMENT", $message);
			}
	...
	<div class="pagination">
	  <ul class="clearfix">
		 <?php $i = 1; while ($i <= $data['pages']){ ?>
				<li><span class=""><a href="<?= (isset($_GET['auth'])) ? "?auth=".$_GET['auth']."&page=" : "?page=" ?><?= $i;?>" class="<?= ($i == $page_num) ? "active" : ""; ?>"><?= $i;?></a></span></li>
		 <?php $i++; } ?>
	  </ul>
	</div>
<?php endif; ?>
