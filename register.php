...
if($validate::requestExists())
{
	...
	if (count($errors) == 0)
	{
		...
		else
		{
			...
			$username = $_POST['username'];
			$message = "Click on the link to activate your account: {$basic_url}/login.php?code={$code}&username={$username}";

			$email = $_POST['email'];
			sendMail($email, "ACCOUNT ACTIVATION", $message);
			// sendMail("benybodipo@gmail.com", "ACCOUNT ACTIVATION", $message);
			redirect("login.php");
		}
	}
}
